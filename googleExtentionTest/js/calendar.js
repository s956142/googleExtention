var Calendar = function() {
  ...
  var initEvent = function() {
    ...
    document
      .querySelector(".right-btn.month-btn")
      .addEventListener("click", function() {
        updateCurrenMonthDate(1);
  ...
  var generateMonth = function(time) {
    ...
    var firstDayIndex =
      set.startWith > firstDay
        ? firstDay - set.startWith + 7
        : firstDay - set.startWith;
  ...
  var setDayWord = function() {
    ...
    for (var i = 0; i < list.length; i++) {
      var index =
        set.startWith + i > 6 ? set.startWith + i - 7 : set.startWith + i;
      list[i].textContent = dayMap[index];
    }
  };
  var setMonthTitle = function(timeObj) {
    document.getElementsByClassName("tmp-month").item(0).textContent =
